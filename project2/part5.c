...
#define ADJUST_THRESHOLD 0.35 // 35% threshold for adjusting time slice

void the_alarm(int signo);
void print_stats();
void update_kernel_times();

int process_cnt = 0;
int process_num = 0;
pid_t * children;
double user_time = 0.0, kernel_time = 0.0, total_time = 0.0, kernel_ratio = 0.0;
...
void the_alarm(int signo) {
    ...
    update_kernel_times(children[process_num]);

    if (kernel_ratio > ADJUST_THRESHOLD && total_time > 0) {
...
void print_stats(){
    system("clear");
    printf("PID\t\tState\t\tUser Time\tKernel Time\tPriority\tThreads\t\tBound\n");
    ...
    for (int i = 0; i < process_cnt; i++) {
        ...
        if (fgets(proc_info, MAX_PROC_INFO_LENGTH, stat_file) != NULL) {
            ...
            update_kernel_times(children[i]);
            
            if (kernel_ratio > ADJUST_THRESHOLD) {
                printf("CPU");
            } else {
                printf("IO");
            }
...
void update_kernel_times(int pid){
    // Open corresponding /proc/<pid>/stat file
    char stat_file_path[256];
    ...
    if (fgets(proc_info, MAX_PROC_INFO_LENGTH, stat_file) != NULL) {
        ...
        while (token != NULL) {
            // Update user and kernel time
            if (index == 14) {
                user_time = atof(token) / sysconf(_SC_CLK_TCK);
            } else if (index == 15) {
                kernel_time = atof(token) / sysconf(_SC_CLK_TCK);
            }
        ...
        // Calculate kernel ratio
        if (total_time > 0)
            kernel_ratio = kernel_time / total_time;

        // Adjust time slice if kernel ratio exceeds threshold
